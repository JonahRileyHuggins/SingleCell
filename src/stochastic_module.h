/*
filename: stochastic_module.h
created by: Jonah R. Huggins
created on: 25-05-08

description: 
Stochastic Simulation Algorithm for Single Cell Model. 
*/
...
const char* stochastic_sbml_path = "../sbml_files/Stochastic.sbml";

//--------------------------Class Declaration-----------------------------//
class StochasticModule {
private:
    std::unique_ptr<SBMLHandler> sbmlHandler; // Pointer to an SBMLHandler instance
    std::vector<std::vector<double>> stoichmat;
    std::vector<std::string> formulas_vector;
    double step;

    std::vector<double> computeReactions(const std::vector<double>& state) {
        /** 
    ...
    double computeReaction(std::string formula_str) {
        /**
    ...
    std::unordered_map<std::string, double>  mapComponentsToValues(const std::string& formula_str) {
        /**
    ...
    std::vector<std::string> tokenizeFormula(const std::string& formula_str) {
        /**
    ...
    std::vector<double> samplePoisson(std::vector<double> initial_reaction_vector) {
        /** 
...
public: // public members accessible from outside:
    ...
    StochasticModule(const std::string& filename = stochastic_sbml_path, 
                    double step_ = 30.0)
         : sbmlHandler(filename),
            step(step_) {
    ...
    void setModelState(const std::vector<double>& state) {
        /**
    ...
    std::vector<double> runStochasticStep(const std::vector<double>& stochastic_state) {
        /**
        ...
        */
        ...
        std::vector<double> v = computeReactions(stochastic_state);
        ...
        std::vector<double> new_state(stochastic_state.size());
        for (size_t i = 0; i < stochastic_state.size(); ++i) {
            double delta = 0.0;
            for (size_t j = 0; j < r.size(); ++j) {
                delta += stoichmat[i][j] * r[j];
            }
            new_state[i] = std::max(stochastic_state[i] + delta, 0.0);
        }
